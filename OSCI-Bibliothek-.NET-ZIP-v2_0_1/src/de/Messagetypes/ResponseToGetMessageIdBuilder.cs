...
{
    /// <summary><H4>ResponseToGetMessageId-Parser</H4>
    /// 
    ...
    /// <p>Version: 2.0.1</p>
    /// </summary>
    public class ResponseToGetMessageIdBuilder
        : OsciMessageBuilder
    {
        private static readonly Log _log = LogFactory.GetLog(typeof(ResponseToGetMessageIdBuilder));
        ...
        /// </param>
        public ResponseToGetMessageIdBuilder(OsciEnvelopeBuilder envelopeBuilder)
            : base(envelopeBuilder)
        {
            Msg = new ResponseToGetMessageId(envelopeBuilder.DialogHandler, true);
        }
        ...
        {
            _log.Trace("Start Element RspFwdDel: " + qName);
            ...
            else if (InsideBody)
            {
                ...
                {
                    CurrentElement = new System.Text.StringBuilder();//("".ToCharArray());
                                                                     //nothing to do
                }
            }
            else if (InsideHeader)
            {
                ...
                else if (localName.Equals("responseToGetMessageId") && uri.Equals(OsciXmlns))
                {
                    //nothing to do
                }
            ...
            {
                throw new SaxException("Nicht vorgesehens Element! Element Name:" + qName);
            }
        }
